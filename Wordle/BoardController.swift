...
class BoardController: NSObject,
    ...
    private func applyNumLettersSettings(with settings: [String: Any]) {
        // START YOUR CODE HERE
        // ...
        if let numLetters = settings[kNumLettersKey] as? Int { //if we let numLetter = settings[] as an int, then execute block. if not do anything
    ...
    private func applyThemeSettings(with settings: [String: Any]) {
        // START YOUR CODE HERE
        //print("Before:", goalWord)
        if let theme = settings[kWordThemeKey] as? String{
            if let new_theme = WordTheme(rawValue: theme){
                goalWord = WordGenerator.generateGoalWord(with: new_theme)
            }
        }
        //print("After:", goalWord)
        // END YOUR CODE HERE
    }
